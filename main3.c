...
void draw_textbox(Display *display, Window win, GC gc, TextBox *textbox, int draw_cursor) {
    ...
    if (draw_cursor && textbox->active) {
        int cursor_x = textbox->x + 5 + XTextWidth(XQueryFont(display, XGContextFromGC(gc)), textbox->text, textbox->cursor_position);
        XDrawLine(display, win, gc, cursor_x, textbox->y + 5, cursor_x, textbox->y + textbox->height - 5);
    }
}
...
int main() {
    ...
    Cursor cursor;
    int draw_cursor = 1;
    TextBox textbox1 = {MARGIN, MARGIN, TEXTBOX_WIDTH, TEXTBOX_HEIGHT, "", 0, 0, 0};
    TextBox textbox2 = {MARGIN, 2 * MARGIN + TEXTBOX_HEIGHT, TEXTBOX_WIDTH, TEXTBOX_HEIGHT, "", 0, 0, 0};
    ...
    XSelectInput(display, window, ExposureMask | ButtonPressMask | KeyPressMask | KeyReleaseMask);
    ...
    while (1) {
        while (XPending(display) > 0) {
            ...
            if (event.type == Expose) {
                draw_textbox(display, window, gc, &textbox1, draw_cursor);
                draw_textbox(display, window, gc, &textbox2, draw_cursor);
                ...
                if (active_textbox) {
                    ...
                    if (keysym == XK_BackSpace) {
                        if (active_textbox->text_len > 0) {
                            ...
                            active_textbox->cursor_position = active_textbox->text_len;
                        }
                    ...
                    draw_textbox(display, window, gc, active_textbox, draw_cursor);
                }
            }
        }
        TextBox *active_textbox = textbox1.active ? &textbox1 : textbox2.active ? &textbox2 : NULL;
